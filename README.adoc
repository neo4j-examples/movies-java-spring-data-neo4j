...
In our case, we use the repository from a `MovieService` to compute a different graph representation for the visualization.
The service is then injected into our main Boot application, which also doubles as `@RestMvcController` that exposes the `/graph` endpoint.
...
* Persistence Access:       Spring-Data-Neo4j 5.3x5
* Database:                 Neo4j-Server 3.5.x
